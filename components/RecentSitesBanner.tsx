...
export default function RecentSitesBanner() {
  ...
  const fetchRecentSites = async () => {
    try {
      ...
      const validSites = (response.data || []).filter(
  ...
  return (
    <div className="w-full h-8 overflow-hidden bg-black relative group">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#00ff9d] to-transparent opacity-0" />
    </div>
  );
}